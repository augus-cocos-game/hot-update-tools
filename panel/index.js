        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                ...
                _packageVersion() {
                    ...
                    // 打包subgame文件
                    let subgame = path.join(this.genManifestDir, "subgame.txt");
                    zip.file("subgame.txt", fs.readFileSync(subgame));
                    // 打包manifest文件
                ...
                OnScanSubGame(){
                    this.m_SubGameMap = {};
                    Editor.assetdb.queryAssets( 'db://assets/resources/SubGame/**\/*', '', function ( err, results ) {//texture
                        for(var i in results){
                            var result = results[i];
                            if(result.path.indexOf('Script') >= 0) continue; //跳过脚本
                            if(result.type == 'folder') continue;
                            
                            var kind = result.path.substring(result.path.indexOf('SubGame')+8);
                            kind = kind.substring(0, kind.indexOf('\\'));
                            this.m_SubGameMap[result.uuid] = kind; 
                        }
                ...
                _genVersion(version, serverUrl, buildResourceDir, genManifestDir) {
                    this._addLog("[Build] 开始生成manifest/txt配置文件....");
                    ...
                    this._addLog("genManifestDir ： "+dest);
                    let readDir = function (dir, obj, subObj) {
                        ...
                        for (let i = 0; i < subpaths.length; ++i) {
                            ...
                            if (stat.isDirectory()) {
                                ...
                                //添加子游戏信息
                                let uuid = relative.substring(relative.lastIndexOf('/')+1,relative.indexOf('.') );
                                if(this.m_SubGameMap[uuid] != null){
                                    if(subObj[this.m_SubGameMap[uuid]] == null) subObj[this.m_SubGameMap[uuid]] = {};
                                    subObj[this.m_SubGameMap[uuid]][relative] = obj[relative]; 
                                }
                    ...
                    readDir(path.join(src, 'src'), manifest.assets, subgame.map);
                    readDir(path.join(src, 'res'), manifest.assets, subgame.map);

                    let destSubgame = path.join(dest, 'subgame.txt');
                    ...
                    // 生成subgame.txt
                    fs.writeFileSync(destSubgame, JSON.stringify(subgame));
                    this._addLog("[Build] 生成 subgame.txt成功");
                    // 生成project.manifest
                ...
                onCopyFileToLocalServer() {
                    ...
                    this._copyFileToDesDir(subgame, this.localServerPath);
                },
                ...
                _getTotalCopyFileNum() {
                    ...
                    return srcNum + resNum + 2 + 2 + 1;// 2个manifest,2个目录(src, res),1个txt
                },
                ...
                onCleanSimRemoteRes() {
                    ...
                    let remoteAsset = path.join(simPath, "take-remote-asset");//bugfix for 1.9.x remote-asset xudongwang
